...
function CreateEntryPage() {
  ...
  const handleClick = (event) => {
    let LeaveConfirmed = window.confirm(
      "Are you sure you want to leave this page? Clicking “OK” will clear this entry."
    );
    if (LeaveConfirmed) {
    } else {
  ...
  const handleSubmit = async (e) => {
    ...
    let SubmitConfirmed = window.confirm(
      'Are you ready to submit your entry? Click "OK" or "Cancel".'
    );
    if (SubmitConfirmed) {
      ...
      try {
        ...
        if (response.status === 201) {
          ...
          alert(
            `ERROR: Failed to create entry! Status code: ${response.status}`
          );
          navigate("/");
        }
      } catch (err) {
    ...
    <>
      ...
      <div className="content">
        <div className="p-container content">
          ...
          <p>
            When you are ready to submit your entry, click the “Submit” button.
          </p>
        </div>
